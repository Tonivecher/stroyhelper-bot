...
def get_list(user_id, list_name):
    ...
    if user_id not in data or "lists" not in data[
            user_id] or list_name not in data[user_id]["lists"]:
        return []
...
def remove_from_list(user_id, list_name, item):
    ...
    if user_id in data and "lists" in data[user_id] and list_name in data[
            user_id]["lists"]:
...
async def show_lists_menu(message: Message, state: FSMContext = None):
    ...
    if user_lists:
        for list_name in user_lists:
            buttons.append([
                InlineKeyboardButton(text=f"📋 {list_name}",
                                     callback_data=f"open_list:{list_name}")
            ])

    buttons.append([
        InlineKeyboardButton(text="✨ Создать новый список",
...
async def process_create_list(callback_query: types.CallbackQuery,
                              state: FSMContext):
    await callback_query.answer()
    await callback_query.message.answer(
        "Введите название для нового списка покупок:")
...
async def process_list_name(message: Message, state: FSMContext):
    ...
    try:
        logger.info(
            f"Попытка создания списка '{list_name}' пользователем {message.from_user.id}"
        )

        # Проверим текущую структуру данных
        current_data = load_shopping_list()
        logger.info(
            f"Текущие данные для пользователя {message.from_user.id}: {current_data.get(str(message.from_user.id), {})}"
        )
        ...
        if created:
            ...
            await message.answer(
                f"⚠️ Список с названием \"{list_name}\" уже существует.")
        ...
        await message.answer(
            "Произошла ошибка при создании списка. Пожалуйста, попробуйте еще раз."
        )
        await state.clear()
...
async def process_open_list(callback_query: types.CallbackQuery,
                            state: FSMContext):
    try:
        ...
        if not items:
            ...
            for item in items:
                if isinstance(
                        item, dict
                ) and 'item' in item and 'quantity' in item and 'unit' in item:
        ...
        buttons = [
            ...
            [
                InlineKeyboardButton(text="🔙 К списку списков",
                                     callback_data="back_to_lists")
            ]
        ]
...
async def process_add_to_list(callback_query: types.CallbackQuery,
    ...
    buttons = [[
        InlineKeyboardButton(text=item, callback_data=f"select_{item}")
    ] for item in items]
    buttons.append([
    ...
    await callback_query.message.edit_text(
        "Выберите материал для добавления в список:", reply_markup=keyboard)


# Обработчик возврата к списку списков
@router.callback_query(lambda c: c.data == "back_to_lists")
async def process_back_to_lists(callback_query: types.CallbackQuery,
                                state: FSMContext):
    await callback_query.answer()
    await show_lists_menu(callback_query.message)
...
async def process_select_material(callback_query: types.CallbackQuery,
                                  state: FSMContext):
    ...
    await callback_query.answer()
    await callback_query.message.edit_text(
        f"Введите количество {item} (число):")
...
async def process_quantity(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(
            f"✅ {item} в количестве {quantity} {unit} добавлен в список \"{list_name}\"!"
        )
        await state.clear()
...
@router.callback_query(
    lambda c: c.data and c.data.startswith("remove_from_list:"))
async def process_remove_from_list_menu(callback_query: types.CallbackQuery,
                                        state: FSMContext):
    list_name = callback_query.data.split(":")[1]
    items = get_list(callback_query.from_user.id, list_name)
    ...
    for item in items:
        if isinstance(item, dict):
            ...
            buttons.append([
                InlineKeyboardButton(
                    text=item_text,
                    callback_data=f"remove_item:{list_name}:{item['item']}")
            ])
        else:
            buttons.append([
                InlineKeyboardButton(
                    text=item, callback_data=f"remove_item:{list_name}:{item}")
            ])
    ...
    await callback_query.message.edit_text(
        f"Выберите товар для удаления из списка \"{list_name}\":",
...
async def process_delete_list(callback_query: types.CallbackQuery,
                              state: FSMContext):
    list_name = callback_query.data.split(":")[1]
